...
export class CommunicationEntity {
  ...
  public cancel() {
    if (this.props.status === "draft" || this.props.status === "scheduled") {
      ...
      throw new Error(
        "Cannot cancel a communication that is not in draft or scheduled status",
      );
    }
  }
  ...
  public updateSubject(subject: string) {
    if (this.props.status !== "draft" && this.props.status !== "scheduled") {
      throw new Error(
        "Cannot update subject after communication is queued, processing, sent, failed or canceled",
      );
    }
  ...
  public updateBody(body: string) {
    if (this.props.status !== "draft" && this.props.status !== "scheduled") {
      throw new Error(
        "Cannot update body after communication is queued, processing, sent, failed or canceled",
      );
    }
  ...
  public updateBodyType(bodyType: "text" | "html") {
    if (this.props.status !== "draft" && this.props.status !== "scheduled") {
      throw new Error(
        "Cannot update body type after communication is queued, processing, sent, failed or canceled",
      );
    }
  ...
  public updateTemplateVariables(
